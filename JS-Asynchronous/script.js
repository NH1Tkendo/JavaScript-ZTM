...
const wait = function (sec) {
  return new Promise(resolve => {
    setTimeout(() => resolve(console.log('Da cho xong 2 giay')), sec * 1000);
  });
};
...
const loadAll = async function (imgArr) {
  const imgs = imgArr.map(async cur => await createImage(cur));
  console.log(imgs);

  const imgsEL = await Promise.all(imgs);
  imgsEL.forEach(cur => cur.classList.add('parallel'));
};
